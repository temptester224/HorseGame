...
<center><a href = "index.php?menu=news" class="stn">Список новостей</a> || <a href = "index.php?menu=news&add" class="stn">Добавить новость</a></center>
<BR />
...
$db->Query("DELETE FROM ".$pref."db_news WHERE id = '$ret_id'");
...
if(isset($_GET["add"])){
	...
	$title = $func->Base64en($title);
	$text = $db->RealEscape($_POST["ntext"]);
	$text = $func->Base64en($text);
	
		if(strlen($title) >= 3){
		
			$db->Query("INSERT INTO ".$pref."db_news (title, news, date_add) VALUES ('$title','$text','".time()."')");
...
$db->Query("SELECT * FROM ".$pref."db_news WHERE id = '$idr' LIMIT 1");
...
if($db->NumRows() != 1){ echo "<center><b>Новость с таким ID не найдена</b></center><BR />"; return;}
	...
	$db->Query("UPDATE ".$pref."db_news SET title = '$title', news = '$text' WHERE id = '$idr'");
...
<form action="" method="post">
<b>Заголовок:</b><BR />
<input type="text" name="title" size="45" value="<?=$func->Base64de($news["title"]); ?>" /><BR /><BR />
<b>Новость:</b><BR />
<textarea name="ntext" cols="78" rows="25"><?=$func->Base64de($news["news"]); ?></textarea><BR />
<center><input type="submit" value="Сохранить" /></center>
</form>
</div>
<div class="clr"></div>	
...
$db->Query("SELECT * FROM ".$pref."db_news ORDER BY id DESC");
...
<?PHP
    ...
    <td align="center"><a href="index.php?menu=news&edit=<?=$data["id"]; ?>" class="stn"><?=$func->Base64de($data["title"]); ?></a></td>
