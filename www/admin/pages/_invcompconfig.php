...
<center><a href = "index.php?menu=invcompconfig" class="stn">Текущий конкурс</a> || <a href = "index.php?menu=invcompconfig&add" class="stn">Добавить Конкурс</a> || <a href = "index.php?menu=invcompconfig&list" class="stn">Завершенные</a></center>
<BR />
<?PHP
...
if(isset($_GET["add"])){
	...
	$db->Query("SELECT * FROM ".$pref."db_invcompetition WHERE status = '0'");
	if($db->NumRows() > 0){
	...
	if(isset($_POST["1m"])){
		...
		if($m1 > $m2 AND $m2 > $m3 AND $m3 > $m4 AND $m4 > $m5){
			...
			$db->Query("INSERT INTO ".$pref."db_invcompetition (1m, 2m, 3m, 4m, 5m, date_add, date_end) VALUES ('$m1','$m2','$m3','$m4','$m5','".time()."','$duration')");
...
if(isset($_POST["cancel"])){
	...
	$db->Query("SELECT * FROM ".$pref."db_invcompetition WHERE status = '0' AND id = '{$cancel}' LIMIT 1");
	if($db->NumRows() == 1){
	
		$db->Query("UPDATE ".$pref."db_invcompetition SET user_1 = '-', user_2 = '-', user_3 = '-',user_4 = '-',user_5 = '-', status = '2' WHERE id = '$cancel'");
		$db->Query("TRUNCATE TABLE ".$pref."db_invcompetition_users");
...
if(isset($_POST["set_ok"])){
	...
	$db->Query("SELECT * FROM ".$pref."db_invcompetition WHERE status = '0' AND id = '{$sok}' LIMIT 1");
	if($db->NumRows() == 1){
		...
		# зачисляем бонус
		$db->Query("SELECT * FROM ".$pref."db_invcompetition_users WHERE points > '0' ORDER BY points DESC LIMIT 3");
		if($db->NumRows() > 0){
			...
			while($data = $db->FetchArray()){
				...
				$db->Query("UPDATE ".$pref."_users_b SET money_p = money_p + '{$money}' WHERE id = '{$user_up_id}'", false, false);
				
			$counter++;
			}
		
		$db->Query("UPDATE ".$pref."db_invcompetition SET user_1 = '".$user_s[1]."', user_2 = '".$user_s[2]."', user_3 = '".$user_s[3]."', user_4 = '".$user_s[4]."', user_5 = '".$user_s[5]."', status = '1' WHERE id = '$sok'");
		$db->Query("TRUNCATE TABLE ".$pref."db_invcompetition_users");
...
$db->Query("SELECT * FROM ".$pref."db_invcompetition WHERE status = 0 LIMIT 1");
if($db->NumRows() == 1){
$comp = $db->FetchArray();	
	?>
...
<BR />
	...
	# Список пользователей
	$db->Query("SELECT * FROM ".$pref."db_invcompetition_users WHERE points > '0' ORDER BY points DESC LIMIT 200");
	if($db->NumRows() > 0){
