<div class="content">
        ...
        <div class="panel panel-default">
         <center>
	<a href = "index.php?menu=sender" class="stn">Список рассылок</a> || <a href = "index.php?menu=sender&add" class="stn">Добавить рассылку</a>
</center>
<center>
<BR />
<?PHP
...
$mess = $func->TextClean($_POST["mess"]);
	...
	if(strlen($title) > 3){
		...
		$db->Query("INSERT INTO ".$pref."db_sender (name, mess, date_add) VALUES ('$title','$mess','".time()."')");
...
if(isset($_POST["del"])){
	...
	$db->Query("DELETE FROM ".$pref."db_sender WHERE id = '".intval($_POST["del"])."'");	
	echo "<center><b>Рассылка удалена</b></center><BR />";

}

$db->Query("SELECT * FROM ".$pref."db_sender ORDER BY id DESC");
