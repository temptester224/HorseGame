...
if(isset($_GET["add"])){
	...
	$db->Query("SELECT * FROM ".$pref."db_competition WHERE status = '0'");
	if($db->NumRows() > 0){
	...
	if(isset($_POST["1m"])){
		...
		if($m1 > $m2 AND $m2 > $m3){
			...
			$db->Query("INSERT INTO ".$pref."db_competition (1m, 2m, 3m, date_add, date_end) VALUES ('$m1','$m2','$m3','".time()."','$duration')");
...
$db->Query("SELECT * FROM ".$pref."db_config WHERE id = '1'");
...
if(isset($_POST["cancel"])){
	...
	$db->Query("SELECT * FROM ".$pref."db_competition WHERE status = '0' AND id = '{$cancel}' LIMIT 1");
	if($db->NumRows() == 1){
	
		$db->Query("UPDATE ".$pref."db_competition SET user_1 = '-', user_2 = '-', user_3 = '-', status = '2' WHERE id = '$cancel'");
		$db->Query("TRUNCATE TABLE ".$pref."db_competition_users");
...
if(isset($_POST["set_ok"])){
	...
	$db->Query("SELECT * FROM ".$pref."db_competition WHERE status = '0' AND id = '{$sok}' LIMIT 1");
	if($db->NumRows() == 1){
		...
		# зачисляем бонус
		$db->Query("SELECT * FROM ".$pref."db_competition_users ORDER BY points DESC LIMIT 3");
		if($db->NumRows() > 0){
		...
		$db->Query("UPDATE ".$pref."db_competition SET user_1 = '".$user_s[1]."', user_2 = '".$user_s[2]."', user_3 = '".$user_s[3]."', status = '1' WHERE id = '$sok'");
		$db->Query("TRUNCATE TABLE ".$pref."db_competition_users");
...
$db->Query("SELECT * FROM ".$pref."db_competition WHERE status = 0 LIMIT 1");
if($db->NumRows() == 1){
$comp = $db->FetchArray();	
	?>
...
<BR />
	...
	# Список пользователей
	$db->Query("SELECT * FROM ".$pref."db_competition_users ORDER BY points DESC LIMIT 200");
	if($db->NumRows() > 0){
