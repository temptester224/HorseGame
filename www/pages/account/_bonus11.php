...
?>
<div class="s-bk-lf">
    <div class="acc-title">Бонус Min Invests</div>
...
Сумма бонуса генерируется случайно от <font color = '#6F4B16'><b>1</b> до <b>50</b></font> Серебра  и выдается пользователям пополнившим баланс на сумму свыше <b>500 рублей</b>.</center>
</p>
...
<center><font color="red"><b>Минимальная сумма пополнения для получения бонуса составляет 200 руб!<b></font></center><BR />
...
$ddel = time() + 60*60*24;
$dadd = time();
$db->Query("SELECT COUNT(*) FROM db_bonus11_list WHERE user_id = '$usid' AND date_del > '$dadd'");
...
$hide_form = false;
	...
	if($db->FetchRow() == 0){
		...
		if(isset($_POST["bonus"])){
			...
			$db->Query("INSERT INTO db_bonus11_list (user, user_id, sum, date_add, date_del) VALUES ('$uname','$usid','$sum','$dadd','$ddel')");

			# Случайная очистка устаревших записей
			$db->Query("DELETE FROM db_bonus11_list WHERE date_del < '$dadd'");
...
<?PHP
$db->Query("SELECT * FROM db_bonus11_list WHERE user = '$uname' LIMIT 1");
if($db->NumRows() > 0){
...
<table width="99%" border="0" align="center">
  ...
  $db->Query("SELECT * FROM db_bonus11_list ORDER BY id DESC LIMIT 20");
